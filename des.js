class DES {
  constructor(key) {
      ...
      const permutated = this.p10.map((index) => this.originalKey[index - 1]);
    ...
    generateK1(left, right) {
      ...
      this.k1 = this.p8.map((index) => joinedKey[index - 1]);
    }
    ...
    generateK2(left, right) {
      ...
      this.k2 = this.p8.map((index) => joinedKey[index - 1]);
    }
...
const encryptaddo = desInstance.encrypt([0,1,0,1,0,1,0,1])
